      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // derivative and putting px 0 will end up with one coefficient
          double epsi = atan(coeffs[1]) - 0.0;
          ...
          int N=(vars.size()-2)/2;
          //start drawing from i = 2, 100ms offset. and end till also N-2
          for(int i=2;i<N-2;i++){
            mpc_x_vals.push_back(vars[i+2]);
            mpc_y_vals.push_back(-vars[i+N+2]);
          }
          ...
          for(int i=0;i<ptsx_eigen.size();i++){
            next_x_vals.push_back(ptsx_eigen[i]);
            next_y_vals.push_back(-ptsy_eigen[i]);
          }
